 ...
 * This greatly simplifies synchronization.
  * 能被DAGScheduler处理的事件类型，DAGScheduler使用事件队列架构，任何线程都可以post一个事件，如一个task完成，一个新job
  * 被提交，有个单独的逻辑线程来读取这些时间并做出决策
  * 这极大的简化了同步
  * 所有的事件都是实现了DAGSchedulerEvent接口的case class
 */
private[scheduler] sealed trait DAGSchedulerEvent//被sealed 声明的 trait仅能被同一文件的的类继承
