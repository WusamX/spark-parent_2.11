...
/**
 * ShuffleMapStages are intermediate（中间） stages in the execution DAG that produce data for a shuffle（为shuffle的reduce侧产生数据）.
 * They occur right before each shuffle operation, and might contain multiple pipelined operations（包括多个pipepline操作）
 * before that (e.g. map and filter). When executed, they save map output files that can later be（多个来源的数据保存到map输出文件为后面的reduce操作做准备）
 ...
 *
 * ShuffleMapStages can also be submitted independently as jobs with DAGScheduler.submitMapStage.（也可以作为独立的job提交给DAGScheduler）
 * For such stages, the ActiveJobs that submitted them are tracked in `mapStageJobs`. Note that
 * there can be multiple ActiveJobs trying to compute the same shuffle map stage.（可以有多个ActiveJob来处理同一个shuffle map stage）
 */
private[spark] class ShuffleMapStage(
    ...
    firstJobId: Int,
    callSite: CallSite,
    val shuffleDep: ShuffleDependency[_, _, _])//描述shuffle的每一阶段
  extends Stage(id, rdd, numTasks, parents, firstJobId, callSite) {

  private[this] var _mapStageJobs: List[ActiveJob] = Nil//计算shuffle map stage的ActiveJob链表

  private[this] var _numAvailableOutputs: Int = 0//保存有多少个map的输出已经就绪

  /**
   * List of [[MapStatus]] for each partition. The index of the array is the map partition id,（一个array元素为一个分区，array的下标为分区的id）
   * and each value in the array is the list of possible [[MapStatus]] for a partition（list里保存的是每个分区对应task多次执行的结果）
